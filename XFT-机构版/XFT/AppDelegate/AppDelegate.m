...
//    [[NSUserDefaults standardUserDefaults] setDouble:amapcoord.latitude forKey:HXUserLatitude];
    ...
    if ([[RCUserAeraManager sharedInstance] loadUserArea]) {
        [RCUserAeraManager sharedInstance].curUserArea.cname = [city stringByReplacingOccurrencesOfString:@"市" withString:@""];
    }else{
        RCUserArea *area = [RCUserArea new];
        area.cname = [city stringByReplacingOccurrencesOfString:@"市" withString:@""];
